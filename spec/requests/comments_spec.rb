...
RSpec.describe '/api/v1/posts/:post_id/comments', type: :request do
  ...
  describe 'create' do
    it 'accept an auth user' do
      login_as(user)
      post "/api/v1/posts/#{new_post.id}/comments", params: default_comment_params.to_json, headers: xhr_header.merge(json_header)
  ...
  describe 'update' do
    it 'accept an owner and a valid update' do
      ...
      patch "/api/v1/posts/#{comment.post.id}/comments/#{comment.id}", params: default_comment_params.to_json, headers: xhr_header.merge(json_header)
    ...
    it 'reject an invalid update' do
      ...
      patch "/api/v1/posts/#{comment.post.id}/comments/#{comment.id}", params: invalid_comment_params.to_json, headers: xhr_header.merge(json_header)
    ...
    it 'protect from CSRF' do
      login_as(comment.user)
      patch "/api/v1/posts/#{comment.post.id}/comments/#{comment.id}", params: default_comment_params.to_json, headers: json_header
      expect(response.status).to eq(403)
    end
  end
